...
const limiter = new RateLimiterRedis({ storeClient: redis, points: 1_000_000, duration: 60, keyPrefix: 'bench:bun-store-rlf:' })
...
await run({
  ...
  isSync: false,
  cleanup: async () => {
    const keys = await redis.keys('bench:bun-store-rlf:*')
