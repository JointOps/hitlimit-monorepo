...
const store = mongoStore({ db, collectionPrefix: 'bench_node_express_hl' })
const mw = hitlimit({
  limit: 1_000_000,
  window: '1m',
  store,
...
await run({
  ...
  isSync: false,
  cleanup: async () => {
    await db.collection('bench_node_express_hl_hits').drop().catch(() => {})
    await db.collection('bench_node_express_hl_bans').drop().catch(() => {})
    await db.collection('bench_node_express_hl_violations').drop().catch(() => {})
    store.shutdown?.()
    await client.close()
  }
})
