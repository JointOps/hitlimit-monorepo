...
const store = postgresStore({ pool, tablePrefix: 'bench_node_hono_hl' })
const app = new Hono()
...
await run({
  ...
  store: 'postgres',
  runtime: 'node',
  versions: { hitlimit: version, hono: JSON.parse(fs.readFileSync(new URL('../../../node_modules/hono/package.json', import.meta.url), 'utf-8')).version, pg: JSON.parse(fs.readFileSync(new URL('../../../node_modules/pg/package.json', import.meta.url), 'utf-8')).version },
  fn: (key) => app.request('/test', { headers: { 'x-forwarded-for': key } }),
  isSync: false,
  cleanup: async () => {
    await pool.query('DROP TABLE IF EXISTS bench_node_hono_hl_hits')
    await pool.end()
  }
})
