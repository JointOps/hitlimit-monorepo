...
import { dragonflyStore } from '../../src/stores/dragonfly.js'
import type { HitLimitStore } from '@joint-ops/hitlimit-types'

const DRAGONFLY_URL = process.env.DRAGONFLY_URL || 'redis://localhost:6382'

describe('DragonflyStore', () => {
  let store: HitLimitStore
  let isAvailable = false

  beforeAll(async () => {
    try {
      const testStore = dragonflyStore({ url: DRAGONFLY_URL })
  ...
  beforeEach(() => {
    ...
    store = dragonflyStore({ url: DRAGONFLY_URL, keyPrefix: `hitlimit:dragonfly:test:${Date.now()}:` })
  })
...
describe('DragonflyStore Lua Scripts', () => {
  let store: HitLimitStore
  let isAvailable = false
  ...
  beforeEach(() => {
    if (!isAvailable) return
    store = dragonflyStore({ url: DRAGONFLY_URL, keyPrefix: `hitlimit:dragonfly:lua:${Date.now()}:` })
  })
