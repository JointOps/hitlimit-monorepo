...
import { buildBody } from '../../src/core/response'
...
describe('buildBody', () => {
  ...
  it('handles response with array values', () => {
    const response = {
      errors: ['rate_limited', 'too_many_requests'],
      codes: [429, 503]
    }
    const body = buildBody(response, baseInfo)

    expect(body.errors).toEqual(['rate_limited', 'too_many_requests'])
    expect(body.codes).toEqual([429, 503])
  })
})
